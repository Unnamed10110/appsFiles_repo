...
$folderBrowser.Description = "Select a directory to use for backup"
$folderBrowser.ShowNewFolderButton = $true

# Show the dialog and check if the user clicked OK
if ($folderBrowser.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
    $backupZipPath = $folderBrowser.SelectedPath
    $backupZipPath = Join-Path $backupZipPath "\windhawk-backup.zip"
    Write-Host "Selected directory: $backupZipPath" -ForegroundColor Green
} else {
    Write-Host "No directory selected. Exiting script."-fregroundColor Red
    exit
}
